...
export default class Game extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            sentence: this.loadSentences(
                levels.levels[storedLevel - 1].sentence
            ),
            words: this.loadWords(levels.levels[storedLevel - 1].sentence),
            hint: levels.levels[storedLevel - 1].hint,
            finished: false,
            showResults: false,
            showHint: false,
            results: [],
    ...
    handleNext = () => {
        if (this.state.currentLevel === this.state.totalLevels) {
            ...
            this.setState({
                currentLevel:
                    this.state.currentLevel < this.state.totalLevels
                        ? this.state.currentLevel + 1
                        : this.state.currentLevel,
                sentence: this.loadSentences(
                    levels.levels[this.state.currentLevel].sentence
                ),
    ...
    loadAnswers() {
        ...
        for (let i = 0; i < container.length; i++) {
            answers.push(
                container[i].querySelectorAll("[data-testid='answer']")
            );
        }
        ...
        for (let i = 0; i < answers.length; i++) {
            ...
            for (let j = 0; j < answers[i].length; j++) {
                sentence.push({
                    word: answers[i][j].innerText,
                    filled: answers[i][j].dataset.filled,
                });
            }
    ...
    getResults() {
        ...
        for (let i = 0; i < answers.length; i++) {
            for (let j = 0; j < answers[i].length; j++) {
                if (answers[i][j].word === "") {
                    renderElements.push(
                        `${j + 1}. slovo v ${i + 1}. vete nebolo vyplnené.`
                    );
                } else if (answers[i][j].word !== this.state.words[i][j]) {
                    renderElements.push(
                        `${answers[i][j].word} nie je správne.`
                    );
                } else {
                        ...
                        <div>
                            ...
                            >
                                <div className="popup__content">
                                    ...
                                    <p>
                                        V každom leveli na vás čaká 10 viet, do
                                        ktorých je potrebné doplniť slová zo
                                        sekcie "Slová, ktoré môžete použiť". Pre
                                        menšie sťaženie, už vyplnené slová v
                                        tomto zozname ostávajú. Slová sa vo
                                        vetách môžu opakovať. Samotné vety sú
                                        povyberané z vybraných gramatických
                                        topicov, avšak sú pomiešané. To znamená,
                                        že v jednom leveli sú napríklad vety s
                                        minulým časom ale aj s frázami. Pri
                                        každom leveli je hint, kde si viete
                                        pozrieť dodatočné pomôcky alebo zobraziť
                                        riešenie.
                                    </p>
                                    <h6>Narábanie so slovami:</h6>
                                    <p>
                                        Slová treba umiestniť do boxu
                                        zvýrazneného slabožltým spodným okrajom.
                                        Pri správnom nasmerovaní slova nad box
                                        sa tento okraj zmení na červený. Po
                                        vypustení kliknutia sa slovo vloží do
                                        daného boxu.
                                    </p>
                                    <h6>Odstránenie slova:</h6>
                                    <p>
                                        Už vyplnené slová sa dajú odstrániť z
                                        boxu kliknutím na slovo.
                                    </p>
                                    <h6>Zobrazenie hodnotenia:</h6>
                                    <p>
                                        Hodnotenie viete zobraziť hocikedy
                                        bezohľadu na to či ste niečo vyplnili
                                        alebo nie. Vyplnenie viet sa dá
                                        vyhodnotiť koľkokrát potrebujete. Viete
                                        si ním kontrolovať správnosť odpovedí ak
                                        dané vety neviete doplniť a tým pádom sa
                                        učiť správne použitie daných slov vo
                                        vetách.
                                    </p>
                                    <h6>Pomôcka:</h6>
                                    <p>
                                        {
                                            levels[this.state.currentLevel - 1]
                                                .hint
                                        }
                                    </p>
